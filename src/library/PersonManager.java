...
public class PersonManager {
	 ...
	 * @param p, the person object itself
	 * @return, the id of the new person
	 */
	public int registerPerson(Person p) { 
		try { 
			personList.put(p.getPersonId(), p);
			System.out.println("[DEBUG]: Added Person (" + p.getClass().getName() + ") with the ID: " + p.getPersonId());
		}
		catch(Exception e) { 
			System.out.println("[DEBUG]: Failed to add Person (" + p.getClass().getName() + "), does an item already exist at ID: " + p.getPersonId() + "?");
			return -1;
		}
	 ...
	 * @return, the list of people in the "database"
	 */
